...
public class CaseDetailRepoImpl implements CaseDetailRepoCustom {
	...
	@Override
	public Page<FdsPosCasesDetail> searchCase(final String sQuery, final String fromdate, final String todate) {
		...
		final Query query = em.createNativeQuery(sbQuery.toString(), FdsPosCasesDetail.class);
		return new PageImpl<FdsPosCasesDetail>(query.getResultList());
	}*/
	...
	public Page<FdsPosCasesDetail> searchCase(final String sQuery, final String fromdate, final String todate) {
		final StringBuilder sbQuery = new StringBuilder(
				" select CD.CASE_NO, CD.TID, CD.RULE_ID, CD.CHECK_DT, CD.UPD_TMS, CD.CASE_STATUS, CD.CHECK_NEW, CD.ASG_TMS, CD.USR_ID, CD.MID, CD.NAME_MID, CD.MCC, CD.NGAY_GD " +
				" from " + SCHEMA + ".FDS_POS_TXN PT left join " +  SCHEMA + ".FDS_POS_CASE_HIT_RULES HR on PT.MA_GD = HR.MA_GD " +
				"                    left join " +  SCHEMA + ".FDS_POS_CASES_DETAIL CD on HR.CASE_NO = CD.CASE_NO " +
				" where 1 = 1" + sQuery);
		sbQuery.append(" order by CD.CHECK_DT");
		final Query query = em.createNativeQuery(sbQuery.toString(), FdsPosCasesDetail.class);
		return new PageImpl<FdsPosCasesDetail>(query.getResultList());
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public Page<FdsPosCasesDetail> searchCasePos(final String sQuery, final String fromdate, final String todate) {
		final StringBuilder sbQuery = new StringBuilder(
				" select DISTINCT CD.CASE_NO, CD.TID, CD.RULE_ID, CD.CHECK_DT, CD.UPD_TMS, CD.CASE_STATUS, CD.CHECK_NEW, CD.ASG_TMS, CD.USR_ID, CD.MID, CD.NAME_MID, CD.MCC, CD.NGAY_GD " +
				" from " + SCHEMA + ".FDS_POS_TXN PT left join " +  SCHEMA + ".FDS_POS_CASE_HIT_RULES HR on PT.MA_GD = HR.MA_GD " +
				"                    left join " +  SCHEMA + ".FDS_POS_CASES_DETAIL CD on HR.CASE_NO = CD.CASE_NO " +
				" LEFT JOIN " +  SCHEMA + ".FDS_POS_CASE_STATUS CS ON CD.CASE_NO=CS.CASE_NO AND (CS.CASE_NO,CS.CRE_TMS) IN (select CASE_NO, max(CRE_TMS) from FDS_POS_CASE_STATUS GROUP BY CASE_NO) " +
				" where 1 = 1" + sQuery);
		sbQuery.append(" order by CD.CHECK_DT");
		final Query query = em.createNativeQuery(sbQuery.toString(), FdsPosCasesDetail.class);
		return new PageImpl<FdsPosCasesDetail>(query.getResultList());
