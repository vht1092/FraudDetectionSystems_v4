...
import com.fds.components.DifferenceForm.Callback;
...
public class SupplyForm extends VerticalLayout {
	private static final long serialVersionUID = 1L;

	private static final String STATUS = "BIL";

	private static final Logger LOGGER = LoggerFactory.getLogger(DiscardForm.class);

	public SupplyForm(final Callback callback, final String caseno, final String cardnumber, final ArrayList<String> arCaseNo) {
		super();
...
//		optionGroup.setStyleName(ValoTheme.OPTIONGROUP_HORIZONTAL);
		...
		descService.findAllByTypeByOrderBySequencenoAsc("ACTION").stream().filter(i->Integer.valueOf(i.getSequenceno())>=100).forEach(item -> {
