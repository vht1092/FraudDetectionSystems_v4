...
public class UserManager extends CustomComponent {
	...
	private BeanItemContainer<FdsPosSysUser> beanSysUserContainer;
	private BeanItemContainer<FdsPosSysRole> beanSysRoleContainer;
	...
	private BeanFieldGroup<FdsPosSysUser> formFieldGroup;
	private FdsPosSysUser selectedUser;
	...
	public UserManager() {
		...
		beanSysUserContainer = new BeanItemContainer<FdsPosSysUser>(FdsPosSysUser.class, sysUserService.findAllUser());
		beanSysRoleContainer = new BeanItemContainer<FdsPosSysRole>(FdsPosSysRole.class, sysRoleService.findAll());
		// Grid role
		gridRole = new Grid();
		...
		// Edit form
		formFieldGroup = new BeanFieldGroup<FdsPosSysUser>(FdsPosSysUser.class);
		...
		return new SelectionListener() {
			...
			public void select(SelectionEvent event) {
				if (gridUser.getSelectedRow() != null) {
					...
					selectedUser = (FdsPosSysUser) gridUser.getSelectedRow();
					showEditForm(selectedUser);
					List<FdsPosSysUserrole> userRole = sysUserroleService.findAllByUserId(selectedUser.getUserid());
					List<FdsPosSysRole> listRole = beanSysRoleContainer.getItemIds();
					MultiSelectionModel multiselectionModel = (MultiSelectionModel) gridRole.getSelectionModel();
					List<FdsPosSysRole> temp = new ArrayList<FdsPosSysRole>();
					for (FdsPosSysRole a : listRole) {
						for (FdsPosSysUserrole b : userRole) {
		...
		return new CommitHandler() {
			...
			public void postCommit(CommitEvent commitEvent) throws CommitException {
				...
				for (Object a : selectedRole) {
					BeanItem<FdsPosSysRole> beanScreenItem = beanSysRoleContainer.getItem(a);
	...
	private void showEditForm(FdsPosSysUser fdsSysUser) {
		if (fdsSysUser == null) {
			fdsSysUser = new FdsPosSysUser();
		}
		BeanItem<FdsPosSysUser> beanFdsSysUser = new BeanItem<FdsPosSysUser>(fdsSysUser);
